...
import { getByPage, deleteItem, saveItem, search } from "../EQAPlanning/EQAPlanningService";
...
class SearchOrgDialog extends React.Component {
  ...
  render() {
    ...
    return (
      <Dialog onClose={handleClose} open={open} PaperComponent={PaperComponent} maxWidth={'sm'} fullWidth={true}>
        <DialogTitle style={{ cursor: 'move' }} id="draggable-dialog-title">
          <span className="mb-20">{t("EQARound.ChoosePlaning")}</span>
        </DialogTitle>
        <DialogContent dividers>
          <Grid item xs={12}>
            ...
            <TextField className="mb-16 mr-10 stylePlaceholder" placeholder={t('EQARound.EnterSearch')} type="text" value={keyword} onChange={this.handleChange} />
            <Button className="mb-16 mr-16" variant="contained" color="primary" onClick={() => this.search(keyword)}>
              {t('Search')}
            </Button>
          </Grid>
          <Grid item xs={12}>
            <MaterialTable title={t("EQARound.EQAPlanningList")} data={this.state.itemList} columns={columns}
