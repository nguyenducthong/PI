(function () {
    ...
    angular.module('EQA.Mixture').controller('SamplePackagingController', SamplePackagingController);

    SamplePackagingController.$inject = [
        '$rootScope',
        '$scope',
        '$http',
        '$timeout',
        ...
        'SamplePackagingService',
        '$uibModal'
    ];
    
    function SamplePackagingController ($rootScope, $scope, $http, $timeout, settings, utils, toastr, service, modal) {
        ...
        $scope.startPackaging = function (bankId) {
            var modalInstance = modal.open({
                animation: true,
                templateUrl: 'sample_packaging_modal.html',
                scope: $scope,
                size: 'md'
            });
            ...
            modalInstance.result.then(function (confirm) {
                if (confirm == 'yes') {
                    toastr.info('Bạn ra ra tube thành công cho ngân hàng huyết thanh EQA-1001-01', 'Thông báo');
                }
            }, function () {
                console.log("cancel");
            });
